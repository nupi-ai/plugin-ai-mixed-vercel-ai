...
describe('eventTypeToConfigKey', () => {
  test('maps known proto event types to config keys', () => {
    ...
    expect(eventTypeToConfigKey('EVENT_TYPE_JOURNAL_COMPACTION')).toBe('journal_compaction');
    expect(eventTypeToConfigKey('EVENT_TYPE_CONVERSATION_COMPACTION')).toBe('conversation_compaction');
  });
